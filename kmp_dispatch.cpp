...
extern int counter;
extern int thread_map[4][200];
extern int flag[100];
extern int TOTAL;
extern int current_parallel;


	extern double read_timer();
	#define NUM_EVENTS 2 
	extern long_long values[NUM_EVENTS];
     	extern int Events[NUM_EVENTS];
	extern double elapsed; /* for timing */
        extern double elapsed_dist; /* for timing */
                        ...
                        kmp_int32 *p_lb, kmp_int32 *p_ub, kmp_int32 *p_st )
{
    ...
    //    printf("\ndispatch next flag[%d]:%d  gtid:%d lb:%d ub:%d st:%d last:%d",gtid,flag[gtid],gtid,*p_lb,*p_ub,*p_st,*p_last);
if(*p_lb==0 && *p_ub==TOTAL)    {/*do nothing*/ }
else
{
char label[30]=" ";
char lower1[20],upper1[20],chunk1[20],graph_id[20];

        thread_map[0][counter]=gtid;
        thread_map[1][counter]=*p_lb;
        thread_map[2][counter]=*p_ub;
        thread_map[3][counter]=(*p_ub-*p_lb) + 1;
        thread_map[4][counter]=current_parallel-1;

snprintf(lower1, 20, "lower:%d", thread_map[1][counter]);
snprintf(upper1, 20, " upper:%d",thread_map[2][counter]);
snprintf(chunk1, 20, " chunk:%d",thread_map[3][counter]);
//snprintf(graph_id, sizeof(graph_id), "%d_%d", counter,gtid);
snprintf(graph_id, 20, "%d_%d_%d", current_parallel-1,*p_lb,*p_ub);
...
//counter++;
node_create(graph_id,label); 				//chunk node create
char source[20];
snprintf(source,sizeof(source), "thread-%d_%d",current_parallel-1,gtid);
edge_create(source,graph_id);				//thread to chunk edge
