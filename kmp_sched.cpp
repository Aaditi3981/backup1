...
//     printf("\nstatic init gtid:%d lb:%d ub:%d st:%d chunk:%d  last:%d",gtid,*plower,*pupper,*pstride,chunk,*plastiter);
	...
	char lower1[20],upper1[20],chunk1[20],graph_id[20];
        ...
        thread_map[3][counter]=(*pupper-*plower) + 1;
	...
	snprintf(graph_id, 20, "%d_%d_%d", current_parallel-1,*plower,*pupper);
	...
	counter++;
	*plower = *plower+*pstride;
	*pupper = *pupper+*pstride;
