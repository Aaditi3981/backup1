...
fprintf(fp1,"<y:GenericNode configuration=\"ShinyPlateNode3\">");
fprintf(fp1,"\n<y:Geometry height=\"79.0\" width=\"281.0\" x=\"71.5\" y=\"110.5\"/>");
fprintf(fp1,"\n<y:Fill color=\"#FF9999\" transparent=\"false\"/>");
fprintf(fp1,"\n<y:BorderStyle hasColor=\"false\" type=\"line\" width=\"1.0\"/>");
fprintf(fp1,"\n<y:NodeLabel alignment=\"center\" autoSizePolicy=\"content\" fontFamily=\"Dialog\" fontSize=\"17\"");
fprintf(fp1," fontStyle=\"plain\" hasBackgroundColor=\"false\" hasLineColor=\"false\" height=\"17.96875\"");
fprintf(fp1," horizontalTextPosition=\"center\" iconTextGap=\"4\" modelName=\"custom\" textColor=\"#000000\" verticalTextPosition=\"bottom\"");
fprintf(fp1," visible=\"true\" width=\"65.171875\" x=\"107.9140625\" y=\"30.515625\"> %s <y:LabelModel>",label);
...
int counter=0;
int thread_map[4][200];
int flag[100];
int TOTAL;
int current_parallel=0;
int first_fork=1;
int total_threads;
int papi_counters[1][200];

void print_thread_map()
{
        int i=0;
        printf("\ngtid\tlower\tupper\tchunk");
        for(i=0;i<counter;i++)
        {
                printf("\ngtid: %d",thread_map[0][i]);
                printf("\tlower: %d",thread_map[1][i]);
                printf("\tupper: %d",thread_map[2][i]);
                printf("\tchunk: %d",thread_map[3][i]);
        }
}
...
// TODO: revert workaround for Intel(R) 64 tracker #96
	...
	total_threads=team->t.t_nproc;	
	//current_parallel++;
   	thread_map[4][counter]=current_parallel;
   	char label[20];
        snprintf(label, sizeof(label) , "fork-%d", current_parallel);
        node_create(label,label);
   	char source[20],dest[20],prev_join[20];
 	for(int i=0;i<team->t.t_nproc;i++)
        { 
        	snprintf(dest, sizeof(dest) , "thread-%d_%d", current_parallel,i);
        	node_create(dest,dest);
        	edge_create(label,dest);
        }
	
	snprintf(prev_join,sizeof(prev_join),"join-%d",current_parallel-1);
        if (first_fork==1)      {       first_fork=0;}
        else
        {
                edge_create(prev_join,label);
...
)
{
	...
	PAPI_stop_counters(values, NUM_EVENTS);		//papi stop counter
    	
	elapsed = (read_timer() - elapsed); 
	  
   	printf("INS: %d, CYC: %d, CPI: %f\n", values[0], values[1], ((double)values[1])/((double)values[0]));
	char label1[50],label[20];
        snprintf(label, sizeof(label) ,"join-%d", current_parallel-1);
	snprintf(label1, sizeof(label1) ,"join-%d \nINS: %d, CYC: %d, CPI: %f", current_parallel-1,values[0], values[1], ((double)values[1])/((double)values[0]));
        node_create(label,label1);
   	char source[20],dest[20];
 		for(int i=0;i<team->t.t_nproc;i++)
        	{
        		snprintf(dest, sizeof(dest) , "thread-%d_%d", current_parallel-1,i);
        		edge_create(dest,label);
        	}
	}
...
/*
        ...
        for(i=0;i<counter;i++)
        {
        sprintf(source, "%d_%d", thread_map[4][i],thread_map[0][i]);
        sprintf(dest, "%d_%d_%d", thread_map[4][i],thread_map[1][i],thread_map[2][i]);
